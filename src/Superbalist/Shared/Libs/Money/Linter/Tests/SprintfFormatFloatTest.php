...
class SprintfFormatFloatTest extends BaseLinterTest {
	...
	protected function extractMatchingFunctionCallsFromFunctions(array $functions, $function)
	{
		...
		foreach ($functions as $f) {
			if (strcasecmp($f['name'], $function) === 0) {
				...
				$subfunctions = array_filter($f['args'], function($arg) {
					return $arg['type'] === 'function';
				});
				$matching = array_merge($matching, $this->extractMatchingFunctionCallsFromFunctions($subfunctions, $function));
			}
		}
		return $matching;
	}
			...
			if ($isInFunction) {
				if (is_string($token)) {
					...
					if ($token[0] === T_STRING && $this->isFunction($token[1])) {
						...
						$function['args'] = array_merge($function['args'], $this->extractFunctionCallsFromTokens($tokens, true));
	...
	{
		$key = 'function_exists_' . $name;
		return Cache::remember($key, 60, function() use ($name) {
